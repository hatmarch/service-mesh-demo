...
. Stop the loadgen
. Get the istio route by running the loadgen command, but indicating that the call should be made once
+
----
$ $DEMO_HOME/scripts/project-load-gen.sh customer istio
...
----
+
. Open Jaeger and show the error in the last few moments (that represents our request)
====
+
